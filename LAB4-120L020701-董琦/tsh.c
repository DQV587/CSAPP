...
void waitfg(pid_t pid)
{
	...
	while(1){
		//wait for a signal to interrupt
		sigsuspend(&empty_set);
		//if the foregroud job stopped or terminated
		if(fgpid(jobs)==0)
			break;
	}
...
void sigchld_handler(int sig)
{
	...
	//block all signals
	sigprocmask(SIG_BLOCK,&mask_all,&prev_all);
	//check all jobs which terminated or stopped
...
void sigint_handler(int sig)
{
	...
	struct job_t *fjob;
	if((fjob=getjobpid(jobs,pid))!=NULL){
...
void sigtstp_handler(int sig)
{
	...
	struct job_t *fjob;
	if((fjob=getjobpid(jobs,pid))!=NULL){
		//send the sigtgtp signal to the foreground job
